...
function server.setPlayerInventory(player, data)
    ...
    if not data then
        local success, result = pcall(db.loadPlayer, player.identifier)
        data = success and result or {}

        if not success then
            warn(('Failed to load inventory for player.%s (%s); loading an empty inventory.'):format(player.source, player.identifier))
        end
    end
    ...
    if type(data) == 'table' then
        ...
        for _, v in pairs(data) do
            if type(v) == 'table' and v.amount and not v.count then
                v.count = v.amount
                v.metadata = v.metadata or v.info or {}
            ...
            if type(v) ~= 'table' or type(v.name) ~= 'string' or not v.count or not v.slot then
                if server.convertInventory then
                    inventory, totalWeight = server.convertInventory(player.source, data)
                    break
                else
                    warn(('Skipping invalid inventory item for player.%s (%s).'):format(player.source, GetPlayerName(player.source)))
                end
            else
                v.name = v.name:lower()
...
lib.addCommand('fixinv', {
    help = 'Reset your inventory busy state',
}, function(source)
    Player(source).state:set('invBusy', false, true)
end)

lib.addCommand('loadinv', {
    help = 'Reload your player inventory',
}, function(source)
    local player

    if shared.framework == 'qb' and GetResourceState('qb-core') == 'started' then
        local qbPlayer = exports['qb-core']:GetCoreObject().Functions.GetPlayer(source)
        player = qbPlayer and qbPlayer.PlayerData
    end

    if player then
        player.identifier = player.citizenid
        player.name = player.name or ('%s %s'):format(player.charinfo.firstname, player.charinfo.lastname)
