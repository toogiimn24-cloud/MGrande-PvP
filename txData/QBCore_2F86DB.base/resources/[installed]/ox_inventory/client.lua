...
function client.openInventory(inv, data)
    ...
    elseif invOpen ~= nil then
        ...
        local success, resultLeft, resultRight, resultAccessError = pcall(lib.callback.await, 'ox_inventory:openInventory', false, inv, data)

        if success then
            left, right, accessError = resultLeft, resultRight, resultAccessError
        elseif not inv then
            left = {
                id = PlayerData.source,
                label = PlayerData.name or 'Inventory',
                type = 'player',
                slots = shared.playerslots,
                weight = PlayerData.weight or 0,
                maxWeight = PlayerData.maxWeight or shared.playerweight
            }
        else
            return lib.notify({ id = 'inventory_right_access', type = 'error', description = locale('inventory_right_access') })
        end
