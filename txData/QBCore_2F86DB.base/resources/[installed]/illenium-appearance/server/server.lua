local outfitCache = {}
local uniformCache = {}
local outfitSchemaReady = false

local function ensureOutfitSchema()
    if outfitSchemaReady then
        return true
    end
    ...
    if not ok then
        print(("[illenium-appearance] Failed to prepare player_outfits table: %s"):format(tostring(err)))
        return false
    end

    outfitSchemaReady = true
    return true
end

CreateThread(ensureOutfitSchema)
...
local function getOutfitsForPlayer(citizenid)
    ...
    if not citizenid or not ensureOutfitSchema() then
        return
    end

    local result = Database.PlayerOutfits.GetAllByCitizenID(citizenid) or {}
    for i = 1, #result, 1 do
        local okComponents, components = pcall(json.decode, result[i].components or "[]")
        local okProps, props = pcall(json.decode, result[i].props or "[]")

        outfitCache[citizenid][#outfitCache[citizenid] + 1] = {
            id = result[i].id,
            name = result[i].outfitname,
            model = result[i].model,
            components = okComponents and components or {},
...
RegisterNetEvent("illenium-appearance:server:saveOutfit", function(name, model, components, props)
    ...
    if not citizenID or not ensureOutfitSchema() then
        lib.notify(src, {
            ...
            description = "Outfit database is not ready",
            type = "error",
            position = Config.NotifyOptions.position
        })
        return
    ...
    if model and components and props then
        ...
        if not ok then
            print(("[illenium-appearance] Failed to save outfit for %s: %s"):format(tostring(citizenID), tostring(id)))
            lib.notify(src, {
                title = _L("outfits.save.failure.title"),
        ...
        if not id then
            print(("[illenium-appearance] Failed to save outfit for %s: insert returned nil"):format(tostring(citizenID)))
            lib.notify(src, {
                title = _L("outfits.save.failure.title"),
                description = "Failed to save outfit",
                type = "error",
